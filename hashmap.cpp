...
void hashmap::put(int key, int value) {
    int bucketNum = 0;
    bucketNum = hashFunction(key) % nBuckets;

    key_val_pair *current = new key_val_pair();
    current->key = key;
    current->value = value;
    current->next = nullptr;
    key_val_pair* prev = nullptr;
    key_val_pair* cur = buckets[bucketNum];
...
int hashmap::get(int key) const {
    int bucketNum = 0;
    bucketNum = hashFunction(key)%nBuckets;
    key_val_pair *current = buckets[bucketNum];
